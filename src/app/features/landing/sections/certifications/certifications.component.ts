...
@Component({
  ...
  standalone: true,
  imports: [RouterLink, DatePipe, SectionHeaderComponent, GlassmorphismCardComponent, ScrollAnimateDirective, SkeletonLoaderComponent],
  changeDetection: ChangeDetectionStrategy.OnPush,
  template: `
    <section id="certifications" class="py-24 px-6">
      <div class="mx-auto max-w-4xl">
        <app-section-header [title]="t.t('certifications.title')" [label]="t.t('certifications.label')" />
              ...
              <div appScrollAnimate [delay]="i * 80">
                <app-glass-card>
                  <div class="flex items-start justify-between gap-3">
                    ...
                    @if (cert.is_verified) {
                      <span class="inline-flex items-center gap-1 rounded-full bg-emerald-500/10 border border-emerald-500/20 px-2 py-0.5 text-xs text-emerald-400">
                  ...
                  @if (cert.certificate_url) {
                    <a [routerLink]="['/certifications', cert.id]"
